...
{
NcsManager::NcsManager(int max_device_number, Device::LogLevel log_level, const std::string& cnn_type,
...
void NcsManager::initDeviceManager()
{
  ...
  {
    auto ncs_handle = std::make_shared<movidius_ncs_lib::NCS>(device_count_, static_cast<Device::LogLevel>(log_level_),
...
#pragma omp parallel for
    for (int device_index = 0; device_index < device_count_; device_index++)
    {
      ...
      ncs_handle_list_[device_index]->classify();
      ClassificationResultPtr result = ncs_handle_list_[device_index]->getClassificationResult();
      results[i * device_count_ + device_index] = std::make_shared<ClassificationResult>(*result);
    }
  }
  ...
  for (int j = 0; j < parallel_left; j++)
  {
    ...
    results[parallel_group * device_count_ + j] = std::make_shared<ClassificationResult>(*result);
  }
...
#pragma omp parallel for
    for (int device_index = 0; device_index < device_count_; device_index++)
    {
      ...
      ncs_handle_list_[device_index]->detect();
      DetectionResultPtr result = ncs_handle_list_[device_index]->getDetectionResult();
      results[i * device_count_ + device_index] = std::make_shared<DetectionResult>(*result);
    }
  }
  ...
  for (int i = 0; i < parallel_left; i++)
  {
    ...
    results[parallel_group * device_count_ + i] = std::make_shared<DetectionResult>(*result);
  }
