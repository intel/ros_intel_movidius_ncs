  ...
  for (unsigned int i = 0; i < caffe_nets.size(); i++)
  {
    try
    {
      std::shared_ptr<movidius_ncs_lib::NCS> handle =
          std::make_shared<movidius_ncs_lib::NCS>(0, static_cast<movidius_ncs_lib::Device::LogLevel>(0), caffe_nets[i],
      ...
      cv::Mat imageData = cv::imread(ros::package::getPath("movidius_ncs_lib") + "/../data/images/bicycle.jpg");
      handle->loadTensor(imageData);
      if (i < 3)
      {
        handle->classify();
        movidius_ncs_lib::ClassificationResultPtr result = handle->getClassificationResult();
      }
      else
      {
        handle->detect();
        movidius_ncs_lib::DetectionResultPtr result = handle->getDetectionResult();
      }
    }
    catch (...)
    {
